...
class LeaveRequestController extends Controller
{
    ...
    public function store(StoreLeaveRequestRequest $request)
    {
        ...
        if($this->isLeaveAlreadyAppliedForThisDay($request->validated(), $user))
    ...
    protected function isLeaveAlreadyAppliedForThisDay($input, $user)
    {
        ...
        {
            if( Punch::where(['emp_code' => $user->emp_code, 'punch_date' => $input['date'], 'punch_by' => 2 ])->exists() )
                return true;
        }
        else
        {
            ...
            {
                if( Punch::where(['emp_code' => $user->emp_code, 'punch_date' => $dateRange->toDateString(), 'punch_by' => 2 ])->exists() )
                    return true;
            }
        }
    ...
    public function activeMedicalLeaveRequest()
    {
        ...
        $leaveRequests = LeaveRequest::with('user.department', 'user.ward', 'leaveType', 'document')
                            ->whereLeaveTypeId( 7 )
                            ->when(!Auth::user()->hasRole(['Admin', 'Super Admin']),
