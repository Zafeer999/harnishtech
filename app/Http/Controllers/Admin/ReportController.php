...
class ReportController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin.reports.month-wise-report')->with(['dateRanges'=> $dateRanges, 'leavesArray'=> $leavesArray, 'otherLeavesArray'=> $otherLeavesArray, 'empList'=> $empList, 'departments'=> $departments, 'holidays'=> $holidays, 'settings'=> $settings, 'leaveTypes'=> $leaveTypes, 'wards'=> $wards, 'class'=> $class, 'fromDate'=> $fromDate, 'toDate'=> $toDate, 'totalDays'=> $totalDays]);
    }
    ...
    public function musterReport(Request $request)
    {
        ...
        if( $request->month )
        {
            ...
            $empList = User::whereNot('id', $authUser->id)
                    ->select('id', 'clas_id', 'shift_id', 'designation_id', 'ward_id', 'department_id', 'sub_department_id', 'emp_code', 'in_time', 'name', 'is_rotational', 'is_ot', 'work_duration', 'sa_duration', 'is_divyang')
                    ->with([ 'ward', 'department', 'shift', 'designation', 'clas', 'empShifts'=> fn($q)=> $q->whereBetween('from_date', [$fromDate, $toDate])])
                    ->with('punches', fn($q) => $q
        ...
        return view('admin.reports.muster-report')->with([ 'errorMessage'=> $errorMessage, 'empList'=> $empList, 'weekDays'=> $weekDays, 'holidays'=> $holidays, 'settings'=> $settings, 'leaveTypes'=> $leaveTypes, 'departments'=> $departments, 'wards'=> $wards, 'class'=> $class, 'fromDate'=> $fromDate, 'toDate'=> $toDate, 'totalDays'=> $totalDays]);
    }
    ...
    public function deviceLogReport(Request $request)
    {
        ...
        return view('admin.dashboard.device-log-report')->with(['data'=> $data, 'isAdmin'=> $isAdmin, 'departments'=> $departments, 'wards'=> $wards]);
    }
